<template>
  <view v-for="(item, index) in list" :key="index">
    <view class="card mg40">
      ...
      <view class="fs36 mgb20">标题：</view>
      <view class="mgb20"> {{ item.title }}</view>
      <view class="fs36 mgb20">用户名：</view>
      <view class="mgb20"> {{ item.user && item.user.name }}</view>
      ...
      <view class="fs36 mgb20 mgt20">正文：</view>
      <view class="mgb20">{{ item.content }}</view>
...
export default {
  ...
  onLoad(options) {
    cf("getUncheckedArticles", {}, true).then((res) => {
      this.list = res;
    });
  },
  methods: {
